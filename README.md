...
| **Stage 4 — report** | Started. Re-scoped: there is one renderer and it lives in the UI, so the PDF is a print stylesheet over the same page rather than a second template in a second language. **4.1 is done** — the engine and the geography now serialise to two JSON payloads that between them carry everything a report needs, including every factor's source, tier, licence and confidence, the corridor's geometry for the map, and what the client owes the people whose data this used. Ranking, the page itself and the limitations page are 4.2 onward. |
